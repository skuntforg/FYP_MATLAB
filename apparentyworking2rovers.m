...
maxtime = 70;             % Upper simulation timer limit, s
...
psid_mod(1,:) = zeros(2,1);
deltapsi_mod(1,:) = zeros(2,1);
deltav_mod(1,:) = zeros(2,1);
dvelocity_mod(1,:) = zeros(2,1);
rho_mod(1,:) = rho;
rhor_mod(1,:) = zeros(2,1);
resultant_v_mod(1,:) = zeros(2,1);
...
% INITIALISE POSITION IF NOT ZERO`
 x_mod(7) = 0;
 x_mod(8) = 0;
 x_mod2(7) = 3; %ensure same value in xt and yt for initial position!
 x_mod2(8) = 3;
...
%     y = dV(1,1) - dv(1,1);
 ...
 %% Calculate the conservative forces
    ...
    [xt(1,2), yt(1,2), vx(1,2), vy(1,2), vr(1,2), Fa2, Fr2, Fd2, Fx2, Fy2, rho_r(1,2)] = apf(xt(1,2), yt(1,2), vx(1,2), vy(1,2), Xg, Yg, xo, yo, noo, Ka, Kr, Kdamp, m, rho_0, dt);
...
% Establish outputs
    ...
    omegadotout_mod(i,:) = Omegadot_mod;    %%DUPLICATETHIS SECTION
